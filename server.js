...
const earliestAnalysisYear = lastlastlastYear // CHANGEME when you change start of analysis
let LYPredSmooth;
...
const updateFlightData = async () => {
    ...
    for (let d = new Date(earliestAnalysisYear, 0, 1); d <= new Date(yyyy, 11, 31); d.setDate(d.getDate() + 1)) {
    ...
    for (let d = new Date(earliestAnalysisYear, 0, 2); d <= today; d.setDate(d.getDate() + 1)) {
